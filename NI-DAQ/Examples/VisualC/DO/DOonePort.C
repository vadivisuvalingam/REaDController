...
*    DIG_Prt_Config, NIDAQErrorHandler, DIG_Out_Port, NIDAQDelay,
*     NIDAQYield
*
...
void main(void)
{
    ...
    while ((iPattern < iNumLoops) && (iStatus == 0)) {
        ...
        iRetVal = NIDAQErrorHandler(iStatus, "DIG_Out_Port",
         iIgnoreWarning);

        printf(" The digital pattern on port %d is set to (DECIMAL) %d\n", iPort, iPattern);
