// RunTest.cpp: implementation of the CRunTest class.
//
//////////////////////////////////////////////////////////////////////
...
/* Variables associated with Double buffer input ********/
...
/* Variables associated with Double buffer output *******/
	...
	if ((motorSpeed) && (revolutions)){
		...
		holecount = fopen("holecount.csv", "w");
		...
		while ((( (int)software_counter < revolutions) && (iStatus == 0)) && (!m_ProgressIndicator->CheckCancelButton())) {
			...
			if (countZeroComparator >= BELT_ENCODER_HOLES){// && (!waveformPaused)){
			...
			//if ( ( ulCounterValue != positionCount ) && ( countZeroComparator >= 360 ) ){
		...
		wfmGenOne->DIG_Port_Disable(BITS_OFF);
		wfmGenOne->DIG_Port_Reset();
...
void CRunTest::AddFileName(CString name)
{
	filename = name;

}

void CRunTest::AddMotorSpeed(int value)
{
	motorSpeed = value;
}

void CRunTest::AddRevolutions(int value)
{
	revolutions = value;
}

// this function estimates time and cycles left to complete the test based 
// on the time duration of the portion of the test completed and the number 
// of cycles completed.  

void CRunTest::TimeCalculate(double time, int cycles, CTestProgressIndicator* m_ProgressIndicator)
{
	// average time per cycle
	float duration = (float)(time/cycles);	

	// cycles left to complete in the test
	int remainingCycles = m_revolutions - cycles;	
...
/*	// string buffer for seconds
	char secondsBuffer[2];	

	// string buffer for minutes
	char minutesBuffer[2];	

	// string buffer for hours
	char hoursBuffer[2]; 

	// string buffer for cycles
	char cyclesBuffer[5];	
*/
	/* convert the integer values to string values in base 10 format*/
/*	_itoa( seconds, secondsBuffer, 10);
	_itoa( minutes, minutesBuffer, 10);
	_itoa( hours, hoursBuffer, 10);
	_itoa( remainingCycles, cyclesBuffer, 10);
	...
	{
/*		message = " hrs  min  sec";
		message.Insert(10,secondsBuffer);
		message.Insert(5,minutesBuffer);
		message.Insert(0,hoursBuffer);
*/
	...
	{
/*		message = " min  sec";
		message.Insert(5,secondsBuffer);
		message.Insert(0,minutesBuffer);
*/
	...
	{
/*		message = " sec";
		message.Insert(0,secondsBuffer);
*/
