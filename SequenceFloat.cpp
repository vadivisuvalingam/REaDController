// SequenceFloat.cpp: implementation of the SequenceFloat class.
//
//////////////////////////////////////////////////////////////////////
...
void SequenceFloat::ClearAll()
{
	...
	for (int i = 0; i < 12; i++){
		...
		for (int j = 0; j < 6; j++){
			...
			sample[i].AddErase(j, 0.0);
			sample[i].AddExpose(j, 0.0);
			sample[i].AddGrid(j, 0.0);
			sample[i].AddPin(j, 0.0);
			sample[i].AddErase2(j, 0.0);
			sample[i].AddExpose2(j, 0.0);
			sample[i].AddGrid2(j, 0.0);
			sample[i].AddPin2(j, 0.0);
...
void SequenceFloat::Copy(SequenceFloat SeqCopy)
{
